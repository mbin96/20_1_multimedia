...
int cur_key;
double key_freq[] = {4000, 330, 349, 392, 523};
double phs, cur_corl;
...
void Rendering(short *in, short *out, int N, double dir)
{
    ...
    dt = (int)(dL * Samplerate / (2*Vs));
    //임의의 큰값 설정
    max_delay = 100;
    //첫실행시 버퍼에 아무값도 들어있지 않아 오류가 나므로 안나게끔 첫실행시엔 버퍼만 옮겨줌
    if (render > 0)
    {
        if (dir >= 0)
        {
...
DWORD WINAPI RecordingWaitingThread(LPVOID ivalue)
{
    while (1)
    {
        ...
        { 
            //printf("fuck");
            makeStereo(in, out, LEN_BUF / 2, dir); 
            memcpy(header[iplaying].lpData, out, LEN_BUF * sizeof(short));
            break;
        } 
        case SLOC :
        {
            Rendering(in, out, LEN_BUF / 2, sound_loc); 
            memcpy(header[iplaying].lpData, out, LEN_BUF * sizeof(short));
            break;
        }
        case SLOC_:
        {
            Build_Sinewaves(cur_key, in, LEN_BUF);
            Rendering(in, out, LEN_BUF / 2, sound_loc);
            memcpy(header[iplaying].lpData, out, LEN_BUF * sizeof(short));
            break;
        }
        ...
        fwrite(out, sizeof(short), LEN_BUF, fp);
...
int main(int argc, TCHAR *argv[])
{
    ...
    pFormat.nSamplesPerSec = Samplerate/2;
    ...
    */
    ...
    do
    {
        ...
        else if (key == 'g')
        {
            filter_on = 0;
            printf("\n청각 테스트기\n");
            printf("소리가 어느쪽에서 들려오는지 확인해 보세요 ");
            sound_loc = (double)dis(gen);
            render = 0;
            sound_loc = sound_loc * PI / 180;
            filter_on = SLOC;
            printf("\n(1)왼쪽 (2)오른쪽 (3) 가운데\n");
            ...
            if (game == 1) {
                if (sound_loc < -PI / 4) {
                    printf("%lf", sound_loc);
                    printf("정답입니다");
                }else if (sound_loc > -PI / 4 && sound_loc < PI / 4) {
                    printf("%lf", sound_loc);
                    printf("틀렸습니다.가운데입니다\n");
                }else if (sound_loc > PI / 4) {
                    printf("%lf", sound_loc);
                    printf("틀렸습니다.오른쪽입니다\n");
                }
